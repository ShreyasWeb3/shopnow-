  ...
  return (
    <Box display="flex" flexDirection="column">
      <TextField
        ...
        onChange={(e) =>
          handleNewAddress((currentNewAddress) => ({
            ...currentNewAddress,
      ...
      <Stack direction="row" my="1rem">
        ...
        <Button
          variant="text"
          onClick={() => hideAddNewAddressView(handleNewAddress)}
        >
          Cancel
        </Button>
      </Stack>
...
const hideAddNewAddressView = (handleNewAddress) => {
  handleNewAddress((currentNewAddress) => ({
    ...currentNewAddress,
...
const Checkout = () => {
  ...
  const addAddress = async (token, newAddress) => {
    try {
      ...
      const data = { address: newAddress.value };
  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    try {
      const response = await axios.post(
        URL,
        { addressId: addresses.selected._id },
        {
          headers: headers,
        }
      );
      ...
      enqueueSnackbar(
        "Something went wrong. Please check that the backend is running, reachable and returns valid JSON",
        { variant: "error" }
      );
    }
  };
  ...
  const updateBalance = (items) => {
    ...
    const updatedBalance =
      Number(localStorage.getItem("balance")) - totalCartValue;
                ...
                addresses.all.map((address) => (
                  <Stack
                    ...
                    className={`address-item ${
                      address._id === addresses.selected._id
                        ? "selected"
                        : "not-selected"
                    }`}
                    onClick={() =>
